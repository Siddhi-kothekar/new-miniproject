...
// Fetch user activities and their points (new endpoint)
app.get('/api/activities/:userId', async (req, res) => {
  try {
    const activities = await CommonUser.find({ userId: req.params.userId }).select('activity points details');
...
app.post('/api/game-stats', async (req, res) => {
  ...
  if (!userId || attempts === undefined || points === undefined) {
    return res.status(400).json({ message: 'Invalid data' });
  }

  try {
    const stats = new GameStats({ userId, attempts, points, timeTaken });
    await stats.save();
    res.status(201).json({ message: 'Game stats saved successfully', data: stats });
  } catch (error) {
